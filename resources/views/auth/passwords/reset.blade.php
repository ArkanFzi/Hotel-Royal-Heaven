    ...
    <div class="w-full max-w-6xl mx-auto rounded-lg overflow-hidden grid grid-cols-1 md:grid-cols-2 shadow-xl">
        ...
        <div class="p-10 bg-white">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-center text-3xl font-bold mb-2 text-gray-800">Reset Password</h3>
                <p class="text-center text-sm text-gray-500 mb-8">Enter your new password below.</p>
                ...
                <form action="{{ route('password.update') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input name="email" id="email" type="email" required placeholder="Email" value="{{ old('email', $request->email) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('email') border-red-500 @enderror">
                        @error('email')<span class="text-sm text-red-500 mt-1 block">{{ $message }}</span>@enderror
                    </div>

                    <div>
                        <label for="password" class="sr-only">New Password</label>
                        <input name="password" id="password" type="password" required placeholder="New Password" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('password') border-red-500 @enderror">
                    ...
                    <div class="flex items-center justify-between">
                        <a href="{{ route('login') }}" class="text-yellow-500 font-medium hover:text-yellow-600">Back to Login</a>
                        <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg py-3 px-6 font-semibold transition-colors duration-200">Reset Password</button>
                    </div>
