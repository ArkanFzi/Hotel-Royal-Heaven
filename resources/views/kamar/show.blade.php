...
<section class="relative w-full min-h-[600px] lg:h-screen flex items-center overflow-hidden bg-gray-900">
    ...
    style="background: linear-gradient({{ $angle }}deg, {{ $bgHex }} 0%, {{ $bgHex }} {{ $splitPercent }}%, transparent {{ $splitPercent }}.1%, transparent 100%), url('{{ asset("user/detailkamar.jpg") }}'); 
...
@section('content')
{{-- 
    Div utama konten. Hapus div header kosong di awal yang ada di kode sebelumnya. 
    Tambahkan `mt-[-16rem]` untuk menarik konten ke atas, menindih Hero Section.
    Gunakan `relative z-20` agar konten ini berada di atas Hero Section.
--}}
<div class="min-h-full w-full bg-gray-50 relative z-20 mt-[-16rem] sm:mt-[-12rem] lg:mt-[-10rem] pb-10">
    ...
    {{-- Atur padding-top menjadi `pt-8` untuk memberi sedikit jarak ke header di atas. Hapus py-8 dari sini. --}}
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                        <div class="text-center">
                            <div class="flex items-center justify-center w-12 h-12 bg-blue-100 rounded-lg mx-auto mb-2">
                                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z"></path></svg>
                            </div>
                        ...
                        <div>
                            ...
                            @if(isset($totalReviews) && $totalReviews > 0)
                                <div class="flex items-center mt-2">
                                    ...
                                    <span class="ml-2 text-lg font-semibold text-gray-900">{{ number_format($averageRating ?? 0, 1) }}</span>
                                    <span class="ml-1 text-gray-600">({{ $totalReviews ?? 0 }} ulasan)</span>
                                </div>
                            @endif
                        </div>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Start auto-slide
            showSlide(0); // Ensure the first slide is shown on load
            startAutoSlide();
        });
    </script>
</div>
